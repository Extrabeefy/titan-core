...
// GUID is a globally-unique identifier used to identify objects within the game.
type GUID uint64

// MakeGUID constructs a new GUID from a low/high pair.
func MakeGUID(low uint32, high static.HighGUID) GUID {
	return GUID(uint64(high)<<32 | uint64(low))
}

// High returns the high-part of the GUID. This correspondings to a HighGUID value.
func (guid GUID) High() static.HighGUID {
	return static.HighGUID(guid >> 32)
}

// Low returns the low-part of the GUID. This is the part that can be modified by the server.
